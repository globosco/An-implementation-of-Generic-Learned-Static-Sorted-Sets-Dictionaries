...
int main(int argc, char* argv[]){
    ...
    if(!sts::util::Utils<uint32_t>::cmdOptionExists(argv, argv+argc, "-m"))
    {
        ...
        if(strcmp(method, "bfe") && strcmp(method, "bft512") && strcmp(method, "bft32k")){
    ...
    */
    ...
    if(!strcmp(type, "uint64")){
      if(p){
        c_start = std::clock();
        bin::binning::PGMBin<uint64_t> * binnedDSl = new bin::binning::PGMBin<uint64_t>(Al, m, p, methStr);
        c_end = std::clock();
        ...
        if(methStr.compare("bfe") == 0){
            c_start = std::clock();
            sts::search::EytzeingerLayout<uint64_t>::createLayout(Al, Vl,0,m);
            c_end = std::clock();
            timerCon = ((double)c_end-(double)c_start) / (double)CLOCKS_PER_SEC;

            c_start = std::clock();
            for(uint64_t i = 0; i < q; i++){
                res[i] = sts::search::EytzeingerLayout<uint64_t>::branchfreeBS(Vl, Ql[i], m, true);
            }
            c_end = std::clock();
        }else if(methStr.compare("bft512") == 0){
            c_start = std::clock();
            sts::search::BTreesLayout<uint64_t, 512>::createLayout(Al, Vl, m);
            c_end = std::clock();
            timerCon = ((double)c_end-(double)c_start) / (double)CLOCKS_PER_SEC;

            c_start = std::clock();
            for(uint64_t i = 0; i < q; i++){
                res[i] = sts::search::BTreesLayout<uint64_t, 512>::branchfreeBS(Vl, Ql[i], 0, m-1);
            }
            c_end = std::clock();
        }else if(methStr.compare("bft32k") == 0){
            c_start = std::clock();
            sts::search::BTreesLayout<uint64_t,32000>::createLayout(Al, Vl,m);
            c_end = std::clock();
            timerCon = ((double)c_end-(double)c_start) / (double)CLOCKS_PER_SEC;

            c_start = std::clock();
            for(uint64_t i = 0; i < q; i++){
                res[i] = sts::search::BTreesLayout<uint64_t, 32000>::branchfreeBS(Vl, Ql[i], 0, m-1);
            }
            c_end = std::clock();
        }
        if(!noCheck)
      ...
      if(p){
        ...
        c_start = std::clock();
        bin::binning::PGMBin<uint32_t> * binnedDSi = new bin::binning::PGMBin<uint32_t>(Ai, m, p, method);
        ...
        if(!noCheck)
            for(int i = 0; i < q; i++){
                if(Ql[i] != binnedDSi->getElement(res[i])){
                    wrong = true;
                    //   std::cout << "Errore in " << i << endl;
                    //   std::cout << Ql[i] << " " << Al[res[i]] << endl;
                }
        ...
        if(methStr.compare("bfe") == 0){
            c_start = std::clock();
            sts::search::EytzeingerLayout<uint32_t>::createLayout(Ai, Vi,0,m);
            c_end = std::clock();
            timerCon = ((double)c_end-(double)c_start) / (double)CLOCKS_PER_SEC;

            c_start = std::clock();
            for(uint64_t i = 0; i < q; i++){
                res[i] = sts::search::EytzeingerLayout<uint32_t>::branchfreeBS(Vi, Qi[i], m, true);
            }
            c_end = std::clock();
        }else if(methStr.compare("bft512") == 0){
            c_start = std::clock();
            sts::search::BTreesLayout<uint32_t, 512>::createLayout(Ai, Vi, m);
            c_end = std::clock();
            timerCon = ((double)c_end-(double)c_start) / (double)CLOCKS_PER_SEC;
            c_start = std::clock();
            for(uint64_t i = 0; i < q; i++){
                res[i] = sts::search::BTreesLayout<uint32_t, 512>::branchfreeBS(Vi, Qi[i], 0, m-1);
            }
            c_end = std::clock();
        }else if(methStr.compare("bft32k") == 0){
            c_start = std::clock();
            sts::search::BTreesLayout<uint32_t,32000>::createLayout(Ai, Vi,m);
            c_end = std::clock();
            timerCon = ((double)c_end-(double)c_start) / (double)CLOCKS_PER_SEC;

            c_start = std::clock();
            for(uint64_t i = 0; i < q; i++){
                res[i] = sts::search::BTreesLayout<uint32_t, 32000>::branchfreeBS(Vi, Qi[i], 0,m-1);
            }
            c_end = std::clock();
            if(!noCheck)
                for(int i = 0; i < q; i++){
