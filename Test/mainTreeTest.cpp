...
int main(int argc, char* argv[]){
    ...
    */
    ...
    if(out == NULL){
        ...
        fprintf(out, "Name, Type, #Elem, Method, BinPerc, Size, Construction Time, Mean Construction Time, Query Time, Mean Query Time, Mean Bin Time, Mean Search Time, Mean Bin Time, Mean Search Time\n");
    }else{
        fclose(out);
    ...
    if(!strcmp(type, "uint64")){
      if(p){
        cout << "UINT64_T" << endl;
        c_start = std::clock();
        binnedDSl = new bin::binning::FixedBin<uint64_t>(Al, m, p, methStr);
        c_end = std::clock();
