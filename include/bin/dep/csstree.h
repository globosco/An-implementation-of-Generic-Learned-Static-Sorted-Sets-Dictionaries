...
public:
    explicit CSSTree(const std::vector<K> &dataset) : leaves(dataset) {
        ...
        // if(std::is_same<K, uint64_t>::value){
        //     std::cout << "LONG CSS TREE" << std::endl;
        // }
    }
    ...
    inline typename std::vector<K>::const_iterator find(K key) const {
        ...
        auto low = leaves.cbegin();
        auto high = leaves.cend();
        // for (; low < high; ++low) std::cout << *low << std::endl;
        if (n_internal_nodes == 0){
            // std::cout << "NO INTERNAL NODES" << std::endl;
        ...
        auto res =  find_in_leaves(lo, hi, key);
     ...
     */
    size_t height() const {
        // std::cout << "Leaves Number:" << leaves.size() << std::endl;
        return tree_height;
    }
};
