// This file is a wrapping for scientific use of PGM-index <https://github.com/gvinciguerra/PGM-index>.
 ...
 */
template<typename K, size_t EpsilonRecursive = 4, typename Floating = double>
class PGMIndexWrap {
protected:
    ...
    void build(RandomIt first, RandomIt last, size_t epsilon, size_t epsilon_recursive) {
        ...
        // std::cout << "SEGMENT:" << n / (epsilon * epsilon)<< std::endl;
        ...
        // std::cout << "SEGMENT:" << segments.size()<< std::endl;
...
public:
    ...
    /**
     * Returns the total number of segments in the index.
     * @return the number of segments
     */
    size_t segments_total_count() const {
        return segments.empty() ? 0 : segments.size();
    }
...
struct PGMIndexWrap<K, EpsilonRecursive, Floating>::Segment {
    ...
    explicit Segment(const typename pgm::internal::OptimalPiecewiseLinearModel<K, size_t>::CanonicalSegment &cs)
        ...
        slope = cs_slope;
        intercept = std::round((float)cs_intercept);
    }
 ...
 * @tparam Floating the floating-point type to use for slopes
 */
template<typename K, typename Floating = double>
using BinarySearchBasedPGMIndex = PGMIndexWrap<K, 0, Floating>;
