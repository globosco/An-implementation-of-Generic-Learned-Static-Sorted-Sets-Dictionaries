            ...
            static uint64_t branchyIS(T *arr, T x, uint64_t start, uint64_t end)
            {
                ...
                {   
                    ...
                    uint64_t pos = lo + (((double)(hi - lo) /
                                    (arr[hi] - arr[lo])) *
                                    (x - arr[lo]));
                    // cout << "(hi - lo): " << (double)(hi - lo) << endl;
                    // cout << "(arr[hi] - arr[lo]): " << (arr[hi] - arr[lo]) << endl;
                    // cout << "(x - arr[lo]): " << (x - arr[lo]) << endl;
                    // cout << "POS: " << pos << endl;
...
//             y_0 * (x_1 - x_0) * (y_1 - y_2))
             ...
             * 
            */
            static uint64_t karySearch(T *A, T x, uint64_t start, uint64_t end, uint64_t k){
