...
namespace sts::search{
    ...
    template<typename T> class LrLearnedDs{

        private:
            uint64_t epsilon = 0;
            T* A = nullptr;
            double* labels = nullptr;
            double w[2] = {0.0}; //= nullptr;
            uint64_t n = 0;
        ...
        public:
            ...
            LrLearnedDs(T* A, uint64_t n, uint64_t epsilon, double *w){
                this->A = A;
                this->n = n;
            ...
            uint64_t* search(T* Q, uint64_t q, char* method, uint64_t *intervals){
                ...
                for(int i = 0; i<q; i++){
                    ...
                    if(!strcmp(method, "bfs"))
                        ...
                        res[i] = StandardLayout<T>::karySearch(A, Q[i], left, right,3);
